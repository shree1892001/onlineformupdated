...
class MontanaForCORP extends BaseFormHandler {
    ...
    async MontanaForCORP(page,jsonData,payload) {
        try {
            ...
            try {
                ...
                if (errorMessage === "Sorry, this name is already taken. Please provide a different name to continue.") {
                    ...
                    await this.clearFieldWithBackspace(page, 'input#field-field1-undefined');
                    await page.type('input#field-field1-undefined', payload.Name.Alternate_Legal_Name); // Type the new name
                    await this.clearFieldWithBackspace(page, 'input#field-field2-undefined');
            ...
            await this.waitForTimeout(3000)
            const shares_fields = [
                { label: 'Shares Authorized*', value: String(payload.Stock_Details.SI_Number_Of_Shares)
                }
            ];
          ...
          await this.clickButton(page, '.add');
            ...
            const inputData = [
                { selector: '#field-address1-HknebDwim_MAIL', value: payload.Registered_Agent.Mailing_Information.Street_Address },
                { selector: '#field-address2-HknebDwim_MAIL', value: payload.Registered_Agent.Mailing_Information['Address_Line 2']  || " " },

                { selector: '#field-addr-city-HknebDwim_MAIL', value: payload.Registered_Agent.Mailing_Information.City },
                { selector: '#field-addr-zip-HknebDwim_MAIL', value: payload.Registered_Agent.Mailing_Information.Zip_Code }
            ];
            ...
            await this.fillInputByName(page,'ADDR1', payload.Incorporator_Information.Address.Street_Address)
            await this.fillInputByName(page,'ADDR2', payload.Incorporator_Information.Address['Address_Line 2']  || " ")

            await this.fillInputByName(page, 'CITY',  payload.Incorporator_Information.Address.City)
            await this.fillInputByName(page,'POSTAL_CODE',String(payload.Incorporator_Information.Address.Zip_Code))
            await page.evaluate(() => {
              const buttons = Array.from(document.querySelectorAll('button.btn.btn-raised.btn-primary'));
              const saveButton = buttons.find(button => button.textContent.trim() === 'Save');
              if (saveButton) {
