...
class ColoradoFormHandlerLLC extends BaseFormHandler {
    ...
    async fillForm(page, data) {
          ...
          async function performEventsonLandingPage(page) {
            try {
              ...
              async function fillAddress(page, addressData, selectors) {
               ...
               await page.evaluate((addressData)=>{
                const dropdown = document.querySelector('#streetAddress-state')
        const option = Array.from(dropdown.options).find(opt => opt.text === addressData.Payload.Principal_Address.PA_State.replace(
        ...
        if (option) {
          dropdown.value = option.value;
                ...
                if (addressData.Incorporator_Information.Address.Inc_Address_Line1 === addressData.Payload.Principal_Address.PA_Address_Line1) {
    ...
    const fieldRules = {
        ...
        '#streetAddress-state' : {required: false, minLength: 2, maxLength: 50, pattern: /^[\x00-\x7F]+$/},
...
module.exports = ColoradoFormHandlerLLC;
