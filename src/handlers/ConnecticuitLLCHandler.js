...
const {navigateToPage,clickLinkByLabel2, addInput, submitForm, randomSleep } = require('../utils/puppeteerUtils');


async function ConnecticuitLLCHandler(page, jsonData) {
    try {
        ...
        await clickLinkByLabel2(page, 'Domestic Business Corporation (For Profit) and Domestic Limited Liability Company');
        await clickLinkByLabel2(page,'Articles of Organization for a Domestic Limited Liability Company (not for professional service limited liability companies)');
        const LLCfieldinput = [
...
module.exports = ConnecticuitLLCHandler;
